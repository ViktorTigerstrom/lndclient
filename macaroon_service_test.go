...
func TestMacaroonGeneration(t *testing.T) {
	...
	// Firstly, we test that will fail to create a macaroon if we don't
	// set cfg.RequiredPerms, or set the permissions to an empty map.
	ms, err := NewMacaroonService(cfg)
	require.NoError(t, err)

	// We start the macaroon service, which will fail as we haven't set the
	// required permissions.
	require.Error(t, ms.Start())
	require.NoError(t, ms.Stop())

	// We now set the required permissions to an empty map, and ensure that
	// the macaroon service fails to start.
	mockPerms := map[string][]bakery.Op{}
	require.Error(t, ms.Start())
	require.NoError(t, ms.Stop())

	// We add 2 required permissions to the mockPerms map, which would make
	// the previous start attempts succeed if we start the service now.
	mockPerms["/uri1.Test/WriteTest"] = []bakery.Op{firstEntityWrite}
	mockPerms["/uri1.Test/ReadTest"] = []bakery.Op{firstEntityRead}
	cfg.RequiredPerms = mockPerms

	// But we also test that we fail to start the macaroon service if we set
