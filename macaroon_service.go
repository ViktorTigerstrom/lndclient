...
func (ms *MacaroonService) Start() error {
	...
	// If there are no required permissions, then we can't create a macaroon
	// and we will return an error.
	if len(ms.cfg.RequiredPerms) == 0 {
		return fmt.Errorf("the config field RequiredPerms must to be " +
			"set and must contain elements to create a macaroon")
	}
